...
class Evaluator(object):
    ...
    def evaluate(self, model, data_loader, tqdm_progress_bar=None, tqdm_desc_func=None):
        ...
        if not tqdm_desc_func:  # Initialize tqdm description function
            tqdm_desc_func = lambda batch_idx: tqdm_progress_bar.desc
...
class Validator(Evaluator):
    ...
    def evaluate(self, model, val_loader, epoch, tqdm_progress_bar=None):
        ...
        metric_vals = super().evaluate(model, val_loader, tqdm_progress_bar,
                                       lambda batch_idx: tqdm_desc_func(epoch, batch_idx), )
...
class Tester(Evaluator):
    ...
    def evaluate(self, model, test_loader, tqdm_progress_bar=None):
        ...
        metric_vals = super().evaluate(model, test_loader, tqdm_progress_bar, tqdm_desc_func)
